...
post_status = function(msg, color) {
  ...
  document.getElementById("ul_statuses").innerHTML += `<li>
...
class CFSSimulation {
  ...
  async resume_simulation() {
      ...
      if (this.state == ADDING_NEW_PROCESSES) {
        ...
        for (let i in this.processes) {
          ...
          if (this.time >= process.start_time && process.in_tree == false) {
            process.last_run = this.fair_clock;
            this.cfs.add_process(process);
            console.log(this.cfs.rb_tree.to_string());
            process.in_tree = true;
            post_status(
              `Process ${process.name} is being inserted into the tree with a vruntime of ${process.vruntime} (out of ${process.total_time_needed})`,
              process.color
            );
        ...
        let last_run = this.curr_node.data.last_run;
        console.log(`fair clock: ${this.fair_clock} last_run: ${last_run}`);
        // Calculate maximum runtime: it's fair share plus any fair
        // shares it missed
        let fair_share =
          Math.round((1 / N) * this.target_latency) +
          this.fair_clock -
          last_run;
        console.log(
          `last run: ${last_run}, fair clock: ${this.fair_clock}, fair share: ${fair_share}`
        );
        // How long this processes will run for
        let duration = Math.max(mg, fair_share);
        let remaining_time =
          this.curr_node.data.total_time_needed - this.curr_node.data.vruntime;
        duration = Math.min(duration, remaining_time);

        // Update fair clock
        this.fair_clock += Math.round((1 / N) * duration);

        // "Run" the process
        this.curr_node.data.last_run = this.fair_clock;
        this.curr_node.data.vruntime += duration;
        this.curr_node.key = this.curr_node.data.vruntime;

        let color = this.curr_node.data.color;
        post_status(
          `Process ${this.curr_node.data.name} has been scheduled to run for ${duration} time units`,
          color
        );
        ...
        this.state = ADDING_NEW_PROCESSES;
        if (
          this.curr_node.data.vruntime >= this.curr_node.data.total_time_needed
        ) {
          // The process is done
          post_status(
            `Process ${this.curr_node.data.name} has completed at time ${
              this.time
            }. It ran for ${
              this.curr_node.data.total_time_needed
            } time units and waited for ${this.time -
              this.curr_node.data.start_time -
              this.curr_node.data.total_time_needed} time units`,
            this.curr_node.data.color
          );
          ...
          console.log(this.cfs.rb_tree.to_string());
          post_status(
            `Process ${this.curr_node.data.name} is being inserted into the tree with a vruntime of ${this.curr_node.data.vruntime} (out of ${this.curr_node.data.total_time_needed})`,
            this.curr_node.data.color
          );
          anim_add_process(
            this.curr_node.data.name,
            this.curr_node.data.vruntime,
            this.curr_node.data.color
          );
          return;
        }
    ...
    document
      .getElementById("div_summary")
      .setAttribute("style", "display: block");
    document
      .getElementById("div_start_over")
      .setAttribute("style", "display: block");
  }
}
...
get_summary = function(e) {
  ...
  if (simulation != undefined && simulation.state == FINISHED) {
    ...
    for (let i = 0; i < simulation.processes.length; i++) {
      table += `<td class="tg-0pky">${simulation.processes[i].time_finished -
        simulation.processes[i].start_time}</td>`;
    }
    // Normalized Turnaround Time (T_r / T_s)
    table += "</tr><tr><td>T<sub>r</sub> / T<sub>s</sub></td>";
    for (let i = 0; i < simulation.processes.length; i++) {
      table += `<td class="tg-0pky">${(
        (simulation.processes[i].time_finished -
          simulation.processes[i].start_time) /
        simulation.processes[i].total_time_needed
      ).toFixed(2)}</td>`;
    }
    table += "</td></table>";
  }
...
document
  .getElementById("btn_start_over")
  .addEventListener("click", function(e) {
    document
      .getElementById("div_input")
      .setAttribute("style", "display: block");
    document
      .getElementById("div_statuses")
